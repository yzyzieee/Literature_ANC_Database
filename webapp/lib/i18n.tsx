"use client";

import { createContext, useContext, useEffect, useState } from "react";

export type Lang = "en" | "zh";

const dict: Record<Lang, Record<string, string>> = {
  en: {
    "brand.tagline": "Audio research knowledge base",
    "nav.library": "Library",
    "nav.review": "Ratings",
    "nav.new": "New card",
    "nav.export": "Export",
    "nav.settings": "Settings",
    "footer": "PDFs stay in Drive · cards, templates, index and scripts stay in GitHub",

    "home.title": "Audio research knowledge base",
    "home.subtitle": "Collaborative knowledge base · audio / ANC / signal processing",
    "home.official": "official cards",
    "home.pending": "pending",
    "home.rated": "rated literature",
    "home.pendingReview": "Pending review",
    "home.viewQueue": "Open queue →",
    "home.recent": "Recently updated",
    "home.browseAll": "Browse all →",

    "cards.title": "Library",
    "cards.subtitle": "All published cards with instant client-side search and team weights.",
    "cards.search": "Search title / tags / authors / summary …",
    "cards.allTypes": "All types",
    "cards.allDomains": "All domains",
    "cards.unit": "cards",

    "detail.authors": "Authors",
    "detail.citationKey": "Citation key",
    "detail.related": "Related",
    "detail.copy": "📋 Copy for your LLM",
    "detail.edit": "Edit on GitHub ↗",
    "detail.fulltext": "Full text / data",
    "detail.activity": "Activity",
    "detail.auditTrail": "Audit trail",
    "detail.uploadedBy": "Card published by",
    "detail.pdfUploadedBy": "PDF uploaded by",

    "comments.title": "Team comments",
    "comments.subtitle": "Add interpretations, implementation notes, cautions, or links that should accompany this paper in future research.",
    "comments.count": "comments",
    "comments.empty": "No team comments yet.",
    "comments.add": "Add a comment",
    "comments.editing": "Edit your comment",
    "comments.placeholder": "What should teammates or their LLM know about this paper?",
    "comments.publish": "Publish comment",
    "comments.update": "Update comment",
    "comments.edit": "Edit",
    "comments.cancel": "Cancel",
    "comments.saving": "Saving…",
    "comments.saved": "Comment saved. It is already in the Card; the static site will refresh after deployment.",
    "comments.updated": "Comment updated. The Card audit trail records this change.",
    "comments.failed": "Comment failed",
    "comments.edited": "edited",
    "comments.open": "Comments",

    "review.title": "Team literature ratings",
    "review.subtitle": "Your queue only contains unrated papers in your research domains. Use History to revisit and update your ratings.",
    "review.formula": "Literature weight",
    "review.formulaHint": "Recommendation, innovation, and rigor are averaged equally. The result is converted to a 0–100 team weight and written back into the card.",
    "review.name": "Your team name",
    "review.namePh": "e.g. Ziyi",
    "review.nameHint": "Use the same name each time so you update your own rating instead of creating another vote.",
    "review.recommendation": "Recommendation",
    "review.innovation": "Innovation",
    "review.rigor": "Rigor",
    "review.weight": "Team weight",
    "review.votes": "ratings",
    "review.unrated": "not rated",
    "review.submit": "Save my rating",
    "review.update": "Update my rating",
    "review.saving": "Saving…",
    "review.saved": "Rating saved. The card and site will update after the automatic deployment.",
    "review.failed": "Rating failed",
    "review.empty": "No paper cards are available for rating yet.",
    "review.loading": "Loading your rating queue…",
    "review.noDomains": "Choose at least one research domain before rating.",
    "review.openSettings": "Open Settings",
    "review.editDomains": "Edit domains",
    "review.queue": "To rate",
    "review.history": "History",
    "review.queueEmpty": "You have rated every paper currently available in your domains.",
    "review.historyEmpty": "You have not rated any papers yet.",
    "pending.title": "Review queue",
    "pending.subtitle": "Drafts in pending. Review happens in GitHub PRs — the LLM only assists; a human is the final academic judge.",
    "pending.openPRs": "Open PR list ↗",
    "pending.checklist": "Review checklist",
    "pending.c1": "Metadata complete: title, type, tags, and (for papers) a citation key matching Zotero",
    "pending.c2": "Content verified: equations, claims and references check out",
    "pending.c3": "Knowledge value & correct cross-links ([[…]]) to existing cards",
    "pending.c4": "On approval: reviewer sets status to official; CI promotes the card after merge",
    "pending.count": "drafts awaiting review",
    "pending.empty": "Queue is empty 🎉",

    "new.title": "New card",
    "new.subtitle": "Upload a PDF for auto-extraction, review the generated card, then publish it directly to the official library.",
    "new.pdfTitle": "Submit a paper PDF",
    "new.pdfHint": "Reads the full paper text in your browser (no upload size limit), then DeepSeek auto-fills the type, metadata and a card distilled from the whole text. Works on digital text PDFs — scanned/image-only PDFs have no extractable text. Review before submitting.",
    "new.pdfBtn": "Choose PDF & auto-fill",
    "new.pdfBusy": "Reading & drafting…",
    "new.pdfOk": "Auto-filled from the PDF — check the type and every field, then submit. The LLM only assists; you are the final judge.",
    "new.pdfFail": "PDF auto-fill failed",
    "new.pdfNoText": "no selectable text (scanned image PDF?)",
    "new.driveReminder": "Upload the PDF file itself to the shared Google Drive folder and paste its link into the Drive field below.",
    "new.driveAuto": "First generate and review the card. Then archive the confirmed PDF below; it is renamed consistently, checked for duplicates, and its Drive link is filled in automatically.",
    "new.driveOpen": "Open shared Drive folder ↗",
    "new.driveUpload": "Upload to Drive",
    "new.driveUploading": "Uploading…",
    "new.driveUploaded": "Uploaded to Drive — link added below.",
    "new.driveVisionOk": "Uploaded to Drive and re-analysed from the original PDF (figures + equations). Review every field, then submit.",
    "new.driveUploadFail": "Drive upload failed",
    "new.driveDuplicate": "This paper is already in Drive. The existing PDF was reused and its link was added below.",
    "new.pdfOkUpload": "Drafted from the PDF text. Review the metadata and card, then archive the confirmed PDF below.",
    "new.archiveTitle": "Archive the confirmed PDF",
    "new.archiveHint": "Do this after checking the card. The PDF is uploaded in reliable chunks, filed by source type, renamed with a global number and citation key, and checked for an existing DOI/citation key.",
    "new.archiveTarget": "Drive target",
    "new.archiveRequired": "Complete the domain, title, and valid citation key before archiving.",
    "new.driveConfirmUpload": "Confirm card & archive PDF",
    "new.driveArchived": "PDF archived",
    "new.driveOpenFile": "Open archived PDF",
    "new.archiveChanged": "The citation key or source type changed after archiving. Archive again before submitting so the PDF and card stay aligned.",
    "new.submitNeedsArchive": "Archive the selected PDF first. The published card will then include its Drive link.",
    "new.type": "Card type",
    "new.domain": "Domain (research field) *",
    "new.domainPick": "— pick a domain —",
    "new.sourceType": "Source type (for the Drive archive)",
    "new.doi": "DOI (optional metadata auto-fill)",
    "new.fetch": "Fetch",
    "new.fetching": "Fetching…",
    "new.cardTitle": "Title *",
    "new.citationKey": "Citation key (Better BibTeX, used as the file name) *",
    "new.authors": "Authors (comma-separated)",
    "new.year": "Year",
    "new.tags": "Tags (comma/space-separated, lowercase)",
    "new.drive": "Google Drive links (PDF / data, space-separated)",
    "new.notes": "Notes for drafting (optional)",
    "new.notesPh": "Points to emphasise, relevance to our project, sections to expand…",
    "new.fileName": "file",
    "new.bodyLabel": "Body (leave empty for the template skeleton; or draft with DeepSeek then edit)",
    "new.draft": "✨ Draft English card with DeepSeek",
    "new.drafting": "Drafting…",
    "new.blank": "Insert blank template",
    "new.submitPr": "Publish to library",
    "new.submitting": "Submitting…",
    "new.download": "Download .md",
    "new.copyMd": "Copy markdown",
    "new.doiOk": "Metadata fetched from Crossref. Override the citation key with your Zotero / Better BibTeX value.",
    "new.doiFail": "DOI lookup failed",
    "new.draftOk": "Draft generated (DeepSeek) — review section by section before submitting. The LLM only assists; you are the final judge.",
    "new.draftFail": "Drafting failed",
    "new.prOk": "Card published to GitHub; the site is deploying",
    "new.prFail": "Publish failed",
    "new.publishingAs": "Publishing as",

    "export.title": "Export for your LLM",
    "export.subtitle": "Bundle cards, team ratings, and member comments into a prompt for your own LLM, then paste its reference list back here to download the originals. No team API spend.",
    "export.filter": "Filter by title / tags …",
    "export.allTypes": "All types",
    "export.selectAll": "Select shown",
    "export.clear": "Clear",
    "export.hasFulltext": "📎 full-text link",
    "export.selected": "cards selected",
    "export.hint": "Copy, then paste into any LLM (ChatGPT / Claude / Kimi …) as context. Each card carries its team comments, Drive full-text link, and GitHub source link.",
    "export.copy": "Copy bundle",
    "export.download": "Download .md",
    "export.preview": "Preview",
    "get.title": "Get original PDFs from a list",
    "get.subtitle": "Paste the reference list your LLM returned (titles or citation keys). Matching cards appear below with a download button — fetch the originals without digging through Drive.",
    "get.placeholder": "Paste the papers your LLM recommended — titles, citation keys, or its whole reference list…",
    "get.matched": "matched in the library",
    "get.noPdf": "no PDF yet",
    "get.copyLinks": "Copy all download links",

    "copy.copied": "✓ Copied",
    "card.download": "Download original PDF",

    "settings.title": "Account settings",
    "settings.subtitle": "Choose the research domains that define your personal rating queue.",
    "settings.loading": "Loading account…",
    "settings.domainsHint": "Select one or more fields. New unrated papers in these fields will appear in Ratings.",
    "settings.save": "Save my domains",
    "settings.saving": "Saving…",
    "settings.saved": "Settings saved. Your Ratings queue is ready.",
    "settings.failed": "Settings update failed",
    "settings.addMember": "Add team account",
    "settings.addHint": "New accounts are stored in team/members.json and can log in immediately.",
    "settings.accountId": "Account ID",
    "settings.displayName": "Display name",
    "settings.initialDomains": "Initial research domains",
    "settings.add": "Add account",
    "settings.adding": "Adding…",
    "settings.memberAdded": "Account added and ready to use.",

    "login.subtitle": "Choose your team account to continue.",
    "login.account": "Team account",
    "login.pick": "— choose your account —",
    "login.submit": "Enter",
    "login.entering": "Entering…",
    "login.wrong": "Unknown or inactive account.",
    "login.logout": "Sign out",
  },
  zh: {
    "brand.tagline": "音频研究知识库",
    "nav.library": "卡片库",
    "nav.review": "文献评分",
    "nav.new": "新建卡片",
    "nav.export": "导出",
    "nav.settings": "设置",
    "footer": "PDF 留在 Drive · 卡片、模板、索引与脚本留在 GitHub",

    "home.title": "音频研究知识库",
    "home.subtitle": "协作知识库 · 音频 / 主动降噪 / 信号处理",
    "home.official": "正式卡片",
    "home.pending": "待审核",
    "home.rated": "已评分文献",
    "home.pendingReview": "待审核",
    "home.viewQueue": "查看队列 →",
    "home.recent": "最近更新",
    "home.browseAll": "浏览全部 →",

    "cards.title": "卡片库",
    "cards.subtitle": "全部正式卡片，支持客户端即时搜索与团队权重。",
    "cards.search": "搜索标题 / 标签 / 作者 / 摘要 …",
    "cards.allTypes": "全部类型",
    "cards.allDomains": "全部领域",
    "cards.unit": "张卡片",

    "detail.authors": "作者",
    "detail.citationKey": "引用键",
    "detail.related": "相关卡片",
    "detail.copy": "📋 复制喂给你的 LLM",
    "detail.edit": "在 GitHub 上编辑 ↗",
    "detail.fulltext": "全文 / 数据",
    "detail.activity": "活动记录",
    "detail.auditTrail": "完整操作记录",
    "detail.uploadedBy": "Card 发布者",
    "detail.pdfUploadedBy": "PDF 上传者",

    "comments.title": "团队评论",
    "comments.subtitle": "补充对文献的理解、实现经验、注意事项或相关链接，供团队和后续 LLM 调研参考。",
    "comments.count": "条评论",
    "comments.empty": "暂时还没有团队评论。",
    "comments.add": "发表评论",
    "comments.editing": "编辑你的评论",
    "comments.placeholder": "这篇文献有哪些信息值得团队成员或他们的 LLM 特别注意？",
    "comments.publish": "发布评论",
    "comments.update": "更新评论",
    "comments.edit": "编辑",
    "comments.cancel": "取消",
    "comments.saving": "保存中…",
    "comments.saved": "评论已写入 Card；静态网站会在自动部署后同步刷新。",
    "comments.updated": "评论已更新，本次修改也已写入 Card 操作记录。",
    "comments.failed": "评论保存失败",
    "comments.edited": "已编辑",
    "comments.open": "评论",

    "review.title": "团队文献评分",
    "review.subtitle": "待评分列表只显示你研究方向内且尚未评分的论文；已评分论文可在历史中访问并修改。",
    "review.formula": "文献权重",
    "review.formulaHint": "推荐度、创新性和严谨性等权平均，再换算为 0–100 的团队权重，并写回文献 card。",
    "review.name": "你的团队姓名",
    "review.namePh": "例如：Ziyi",
    "review.nameHint": "请每次使用同一个姓名，这样会更新自己的评分，而不是产生重复票。",
    "review.recommendation": "推荐度",
    "review.innovation": "创新性",
    "review.rigor": "严谨性",
    "review.weight": "团队权重",
    "review.votes": "人评分",
    "review.unrated": "尚未评分",
    "review.submit": "保存我的评分",
    "review.update": "修改我的评分",
    "review.saving": "保存中…",
    "review.saved": "评分已保存。自动部署完成后，card 和网站会同步更新。",
    "review.failed": "评分失败",
    "review.empty": "正式库中暂时没有可评分的论文卡。",
    "review.loading": "正在加载你的评分队列…",
    "review.noDomains": "请先选择至少一个研究方向。",
    "review.openSettings": "打开设置",
    "review.editDomains": "修改方向",
    "review.queue": "待评分",
    "review.history": "评分历史",
    "review.queueEmpty": "你已经完成当前研究方向内的全部文献评分。",
    "review.historyEmpty": "你还没有评分过任何论文。",
    "pending.title": "审核队列",
    "pending.subtitle": "pending 中的草稿卡。审核在 GitHub PR 中进行——LLM 只辅助，人是最终学术裁判。",
    "pending.openPRs": "打开 PR 列表 ↗",
    "pending.checklist": "审核清单",
    "pending.c1": "元数据完整：标题、type、tags，论文卡的 citation key 与 Zotero 一致",
    "pending.c2": "内容已核对：公式、结论、引用出处可查",
    "pending.c3": "有知识价值，且与已有卡片正确互链（[[…]]）",
    "pending.c4": "通过后：审核人将 status 改为 official；合并后 CI 自动晋升",
    "pending.count": "张待审卡片",
    "pending.empty": "队列为空 🎉",

    "new.title": "新建卡片",
    "new.subtitle": "上传 PDF 自动解析，核对生成的知识卡后，直接发布到正式文献库。",
    "new.pdfTitle": "提交论文 PDF",
    "new.pdfHint": "在浏览器里读取论文全文（无上传大小限制），再由 DeepSeek 自动判定类型、填好元数据、并基于整篇全文提炼正文。适用于电子版文字 PDF——纯扫描/图片 PDF 无可提取文字。提交前请核对。",
    "new.pdfBtn": "选择 PDF 并自动填充",
    "new.pdfBusy": "读取并起草中…",
    "new.pdfOk": "已根据 PDF 自动填充——请核对类型和每个字段后再提交。LLM 只辅助，人是最终裁判。",
    "new.pdfFail": "PDF 自动填充失败",
    "new.pdfNoText": "PDF 中没有可选文字（可能是扫描图片版）",
    "new.driveReminder": "请把 PDF 文件本身上传到团队共享的 Google Drive 文件夹，并把链接粘到下面的 Drive 字段。",
    "new.driveAuto": "先生成并核对知识卡，再在下方归档确认后的 PDF；系统会统一重命名、检查重复文献，并自动回填 Drive 链接。",
    "new.driveOpen": "打开共享 Drive 文件夹 ↗",
    "new.driveUpload": "上传到 Drive",
    "new.driveUploading": "上传中…",
    "new.driveUploaded": "已上传到 Drive，链接已自动填入。",
    "new.driveVisionOk": "已上传到 Drive，并由 Gemini 读取原 PDF（含图表/公式）重新提炼。请核对每个字段后提交。",
    "new.driveUploadFail": "Drive 上传失败",
    "new.driveDuplicate": "Drive 中已存在这篇文献，已复用原 PDF 并自动填入链接。",
    "new.pdfOkUpload": "已根据 PDF 文字生成草稿。请先核对元数据和知识卡，再在下方归档确认后的 PDF。",
    "new.archiveTitle": "归档确认后的 PDF",
    "new.archiveHint": "请在核对知识卡后执行。PDF 会分块上传，按来源类型归档，以全局编号和 citation key 统一命名，并按 DOI/citation key 检查重复文献。",
    "new.archiveTarget": "Drive 目标",
    "new.archiveRequired": "请先填写领域、标题和有效的 citation key。",
    "new.driveConfirmUpload": "确认卡片并归档 PDF",
    "new.driveArchived": "PDF 已归档",
    "new.driveOpenFile": "打开已归档 PDF",
    "new.archiveChanged": "归档后 citation key 或来源类型发生了变化。请重新归档，确保 PDF 与卡片一致。",
    "new.submitNeedsArchive": "请先归档已选择的 PDF，随后发布的 card 会自动带上 Drive 链接。",
    "new.type": "卡片类型",
    "new.domain": "领域（研究方向）*",
    "new.domainPick": "— 选择领域 —",
    "new.sourceType": "来源类型（用于 Drive 归档）",
    "new.doi": "DOI（可自动抓取元数据）",
    "new.fetch": "抓取",
    "new.fetching": "查询中…",
    "new.cardTitle": "标题 *",
    "new.citationKey": "Citation key（Better BibTeX，作为文件名）*",
    "new.authors": "作者（逗号分隔）",
    "new.year": "年份",
    "new.tags": "标签（逗号/空格分隔，小写）",
    "new.drive": "Google Drive 链接（PDF / 数据，空格分隔）",
    "new.notes": "起草要点提示（可选）",
    "new.notesPh": "想突出的重点、与课题的关联、需要展开的小节…",
    "new.fileName": "文件名",
    "new.bodyLabel": "正文（留空则用模板骨架；可先用 DeepSeek 生成再改）",
    "new.draft": "✨ 用 DeepSeek 生成英文卡片",
    "new.drafting": "起草中…",
    "new.blank": "插入空白模板",
    "new.submitPr": "发布到文献库",
    "new.submitting": "提交中…",
    "new.download": "下载 .md",
    "new.copyMd": "复制 Markdown",
    "new.doiOk": "已从 Crossref 获取元数据。请用 Zotero / Better BibTeX 的 citation key 覆盖建议值。",
    "new.doiFail": "DOI 查询失败",
    "new.draftOk": "草稿已生成（DeepSeek）——请逐节核对后再提交。LLM 只辅助，人是最终裁判。",
    "new.draftFail": "生成失败",
    "new.prOk": "卡片已写入 GitHub，网站正在自动部署",
    "new.prFail": "发布失败",
    "new.publishingAs": "当前发布账号",

    "export.title": "导出给你的 LLM",
    "export.subtitle": "把知识卡、团队评分和成员评论一起打包给你自己的 LLM，再把它给的文献清单粘回这里下载原文。不消耗团队 API 额度。",
    "export.filter": "按标题 / 标签过滤 …",
    "export.allTypes": "全部类型",
    "export.selectAll": "全选当前",
    "export.clear": "清空",
    "export.hasFulltext": "📎 有全文链接",
    "export.selected": "张卡片已选",
    "export.hint": "复制后粘贴到任意 LLM（ChatGPT / Claude / Kimi …）作为上下文；每张卡都带团队评论、Drive 全文链接与 GitHub 源链接。",
    "export.copy": "复制卡片包",
    "export.download": "下载 .md",
    "export.preview": "预览",
    "get.title": "按清单下载原文 PDF",
    "get.subtitle": "把 LLM 返回的文献清单（标题或 citation key）粘进来,匹配到的卡片会列在下面并带下载按钮——不用进 Drive 翻就能拿原文。",
    "get.placeholder": "粘贴 LLM 推荐的论文——标题、citation key,或它给的整段参考文献列表…",
    "get.matched": "篇在库中匹配到",
    "get.noPdf": "暂无 PDF",
    "get.copyLinks": "复制全部下载链接",

    "copy.copied": "✓ 已复制",
    "card.download": "下载原文 PDF",

    "settings.title": "账号设置",
    "settings.subtitle": "选择研究方向，用于生成你的个人评分队列。",
    "settings.loading": "正在加载账号…",
    "settings.domainsHint": "可选择一个或多个领域；这些领域中新加入且尚未评分的论文会出现在评分页。",
    "settings.save": "保存我的方向",
    "settings.saving": "保存中…",
    "settings.saved": "设置已保存，个人评分队列已经更新。",
    "settings.failed": "设置更新失败",
    "settings.addMember": "新增团队账号",
    "settings.addHint": "新账号会写入 team/members.json，并可立即登录。",
    "settings.accountId": "账号 ID",
    "settings.displayName": "显示名称",
    "settings.initialDomains": "初始研究方向",
    "settings.add": "新增账号",
    "settings.adding": "新增中…",
    "settings.memberAdded": "账号已新增，可以立即使用。",

    "login.subtitle": "选择你的团队账号后进入。",
    "login.account": "团队账号",
    "login.pick": "— 选择账号 —",
    "login.submit": "进入",
    "login.entering": "进入中…",
    "login.wrong": "账号不存在或已停用。",
    "login.logout": "退出登录",
  },
};

interface Ctx {
  lang: Lang;
  setLang: (l: Lang) => void;
  t: (k: string) => string;
}

const LangContext = createContext<Ctx>({ lang: "en", setLang: () => {}, t: (k) => k });

export function LangProvider({ children }: { children: React.ReactNode }) {
  const [lang, setLangState] = useState<Lang>("en");

  useEffect(() => {
    const saved = localStorage.getItem("kb-lang");
    if (saved === "en" || saved === "zh") setLangState(saved);
  }, []);

  useEffect(() => {
    document.documentElement.lang = lang === "zh" ? "zh-CN" : "en";
  }, [lang]);

  const setLang = (l: Lang) => {
    setLangState(l);
    localStorage.setItem("kb-lang", l);
  };

  const t = (k: string) => dict[lang][k] ?? dict.en[k] ?? k;

  return <LangContext.Provider value={{ lang, setLang, t }}>{children}</LangContext.Provider>;
}

export const useLang = () => useContext(LangContext);

/** Inline translated string for use inside server components. */
export function T({ k }: { k: string }) {
  const { t } = useLang();
  return <>{t(k)}</>;
}

export function LangToggle() {
  const { lang, setLang } = useLang();
  return (
    <button
      className="lang-toggle"
      onClick={() => setLang(lang === "en" ? "zh" : "en")}
      aria-label={lang === "en" ? "切换到中文" : "Switch to English"}
      title={lang === "en" ? "切换到中文" : "Switch to English"}
    >
      {lang === "en" ? "中文" : "EN"}
    </button>
  );
}
